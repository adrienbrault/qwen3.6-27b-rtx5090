...
# flan/patches-nvfp4kv stack (PR #49891 rebase + linear-V-scale store overlay).
...
# Gauntlet: see flan/patches-nvfp4kv/README.md (pool -> depth needles cold/warm,
...
sudo docker image inspect "$IMAGE" >/dev/null 2>&1 || { echo "FATAL: image $IMAGE missing — build flan/patches-nvfp4kv/Dockerfile.nvfp4kv first" >&2; exit 1; }
