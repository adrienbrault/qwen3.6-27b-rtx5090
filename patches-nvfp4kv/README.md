...
- Launcher: `flan/launch-qwen38-nvfp4kv.sh` (:8029/vllm-exp, plain, FLASHINFER forced,
...
rsync -a flan/patches-nvfp4kv/ 10.76.10.5:/srv/qwen5090/nvfp4kv-build/
# 2) build (~5-10 min: git csrc checkout + one nvcc + patch), no GPU needed
ssh 10.76.10.5 'cd /srv/qwen5090/nvfp4kv-build && sudo docker build -f Dockerfile.nvfp4kv -t vllm-qwen38:nvfp4kv .'
# 3) boot the audition engine (daily/eval engine must be down; first boot JITs FA2 nvfp4 kernels ~15 min)
ssh 10.76.10.5 'bash -s' < flan/launch-qwen38-nvfp4kv.sh
```
...
2. **Depth needles, cold + warm**, `flan/vllm-turboquant/kv_quality.py` style, 9K/20K/40K/60K
